...
# Override to only be used in this management command
class DrupalCourseMarketingSiteDataLoader(CourseMarketingSiteDataLoader):
    ...
    def __init__(self, partner, api_url, access_token, token_type, max_workers, is_threadsafe, course_ids, **kwargs):
        ...
        super(CourseMarketingSiteDataLoader, self).__init__(
    ...
    def process_node(self, data):
        ...
        end_date = datetime.datetime.fromtimestamp(int(field_course_end_date)) \
            if field_course_end_date else datetime.datetime.min
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            dest='course_ids',
            required=True,
            help='The Drupal course keys to populate.'
        )
        ...
        parser.add_argument(
            ...
            dest='partner_code',
            required=True,
            help='The partner short code to run this for'
        )
    ...
    def handle(self, *args, **options):
        ...
        token_type = 'JWT'
        for partner in Partner.objects.filter(short_code=options.get('partner_code')):
            try:
            ...
            execute_loader(
                ...
                False,
                set(options.get('course_ids', '').split(',')),
                **kwargs
            )
